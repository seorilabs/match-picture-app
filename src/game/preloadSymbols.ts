...
/**
 * 덱에서 쓰일 수 있는 심볼 이미지 전체를 팩 단위로 미리 디코딩해 둡니다.
 * 라운드 전환 때 처음 보는 심볼이 늦게 뜨는 현상을 막기 위한 것으로,
 * 메인 스레드를 막지 않도록 idle 타이밍에 팩당 한 번만 실행합니다.
 */
export function preloadSymbolImages(pack: SymbolPack): void {
  if (
    preloadedPacks.has(pack.id) ||
    typeof window === "undefined" ||
    typeof Image === "undefined"
  ) {
    return;
  }
  preloadedPacks.add(pack.id);

  const run = () => {
    for (let i = 1; i <= SYMBOL_COUNT; i++) {
      const image = new Image();
      image.decoding = "async";
      image.src = symbolSrc(pack, symbolName(i));
