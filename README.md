...
`npm run dev`는 Granite 개발 서버를 실행합니다. 일반 브라우저에서도 게임 자체는 동작하며, Apps in Toss 브릿지(Storage, share, requestReview 등)는 토스 앱 또는 샌드박스 앱에 연결했을 때만 정상 동작합니다.
...
src/
  ...
  ait/                 Apps in Toss 브릿지 어댑터(스토리지, 공유, 리뷰 등)
